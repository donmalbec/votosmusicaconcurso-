-- DRAFT ONLY - do not run until the organizer approves the deployment window.
--
-- Forward-only hardening for public.votes.
-- This does not delete, update, or invalidate historical votes. It closes direct
-- public access and rejects new duplicate/invalid inserts from this point on.
-- New app code stores a server-generated 64-char HMAC device_id, so the device
-- constraint below is compatible with both app and database enforcement.
